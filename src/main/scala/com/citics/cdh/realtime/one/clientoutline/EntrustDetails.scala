...
object EntrustDetails {
  ...
  val conf = new SparkConf().setAppName("crmClientOutline_one_entrustDetails")
  val logger = LoggerFactory.getLogger(getClass)

  val schemaInsert = Utils.getAvroSchema(Utils.schemaEntrustInsert)
  val schemaUpdate = Utils.getAvroSchema(Utils.schemaEntrustUpdate)
  val recordInjectionInsert = GenericAvroCodecs.toBinary[GenericRecord](schemaInsert)
  val recordInjectionUpdate = GenericAvroCodecs.toBinary[GenericRecord](schemaUpdate)

  val schemaRDD = StructType(schemaInsert.getFields.map(f => StructField(f.name(), StringType, nullable = true)))
  ...
  def main(args: Array[String]): Unit = {
    ...
    sc.setLogLevel("WARN")
    val kafkaReader = new KafkaReader[String, Array[Byte], StringDecoder, DefaultDecoder](ssc, Utils.brokerList, Utils.topicClientOutline,
                                                                                          Utils.hbaseTKafkaOffset, Utils.hbaseHosts, Utils.hbasePort)

    val kafkaStream = kafkaReader.getKafkaStream()

    val insertRecords = kafkaStream.filter(x => x._1 == "ogg_entrust_insert").map(x => {
      val avroRecord = recordInjectionInsert.invert(x._2).get
      Row.fromSeq(schemaInsert.getFields.map(f => avroRecord.get(f.name()).asInstanceOf[Utf8].toString))
    }).persist()

    val updateRecords = kafkaStream.filter(x => x._1 == "ogg_entrust_update").map(x => {
      recordInjectionUpdate.invert(x._2).get.toString
    }).persist()
    ...
    insertRecords.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        val entrust_details = hvc.createDataFrame(rdd, schemaRDD)
...
//        "op_entrust_way", "entrust_bs", "money_type", "stock_type", "init_date")) {
        ...
        entrust_details.printSchema()
        entrust_details.show(10)
...
//                    tableMapping.put(putMapping)
                    ...
                    //实时汇总部分
                    val entrustKey = String.format(Utils.redisAggregateEntrustKeyOne, init_date, staff_id)
