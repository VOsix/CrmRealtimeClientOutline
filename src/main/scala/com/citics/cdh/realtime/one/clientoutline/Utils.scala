...
object Utils {
  ...
  //avro schema
  val schemaEntrustInsert = "/realtime/avro/ogg_entrust_insert.schema"
  val schemaEntrustUpdate = "/realtime/avro/ogg_entrust_update.schema"
  ...
  val topicClientOutline = "realtime-client-outline"
  ...
  val hbaseTEntrustDetails  = "realtime:client_outline_entrust_details_one"
  val hbaseTEntrustMapping  = "realtime:client_outline_entrust_mapping_one"
  ...
  val redisAggregateEntrustKeyOne  = "realtime:crm:aggregate:entrust:one:init_date:%s:staff_id:%s"
  ...
  def getAvroSchema(path: String): Schema = {
    val fs = FileSystem.get(new java.net.URI("hdfs://nameservice1"), new Configuration())
    ...
    logger.warn(s"schema: ${schema.toString()}")
    schema
  }
